...
namespace robot{
	...
	/** 
         * import the scene from a given file
	 * this is different with function above is this function use for import hetero_nao rsg file
	 * this way may be not the best one!
	 * add by yao  2013/05/23
         * 
         * @param filename the configuration file
         * @param node the root node
	 * @param heteroType the type of hetero nao
         * 
         * @return if the importing successful
         */
        bool importHetero(const string& filename, char heteroType, shared_ptr< Node<Device> > root);
	...
	/**
	 * evaluate the expression of choose different size by different nao type
	 * the function is for expression like "NaoType["$type"]['ThighRelHip2_Z']"
	 * add in 2013/5/19  by yao
	 *
	 * @param sexp the input S expression
         * @param defs the current defines
         * 
         * @return the results value
	 */    
	float evalHetero(const sexp_t* sexp, const TDefinesMap& defs)
	    throw(ClassException<RSGImporter>);
...
#define DECLARE_DEVICE_INVOKE(class_name)       \
        ...
        DECLARE_DEVICE_INVOKE(AgentCollisionHandler);
