...
namespace robot{
    ...
    bool RSGImporter::importHetero(const string& filename, char heteroType, shared_ptr< Node<Device> > root) {
        TDefinesMap defs;
	...
	sexp_t* t = sexp->list;
	string title;
	SexpParser::parseValue(t, title);
	if("importScene" == title) {
	  t = t->next;
	  SexpParser::parseValue(t, title);
	 
	  char *paramt = new char[2];
	  paramt[0] = heteroType;
	  paramt[1] = '\0';
          t = t->next;
	  t->val = paramt;
	  
	  return import(title, t, defs, root);
	} else {
	  return false;
	}
        ...
        throw(ClassException<RSGImporter>)
    {
        ...
        while( sexp ){
            ...
            string name(t->val);
            if ( "def" == name ) {
	      define(t->next,localDefs);
	    } else if ( "nd" == name ) {
	      importNode(t->next, localDefs, node);
	    } else if ( "templ" == name ) {
	      readTemplate(t->next, args, localDefs);
	    }
            else {
        ...
        throw(ClassException<RSGImporter>)
    {
        ...
        else if ( SexpParser::isList(sexp) )
        {
            ...
            if( "eval" == op )
            {
	      //test code by yao 2013/05/21
	      /// ////////
		string temp;
		SexpParser::parseValue(sexp->next, temp);
		
		if("NaoType[" == temp) {
		 value = lexical_cast<DATATYPE>( evalHetero(sexp->next,defs) );
		 return true;
		} else {
    ...
    float RSGImporter::evalHetero(const sexp_t* sexp, const RSGImporter::TDefinesMap& defs) 
	throw(ClassException<RSGImporter>)
    {
        sexp = sexp->next;
	if ( !sexp ) {
	...
	string heteroType;
	if( !SexpParser::parseValue(sexp, heteroType) ) {
	  throw (ClassException<RSGImporter>(" can not parse: "));
	  return 0;
	} 
	
	heteroType.erase(0, 1);
	float result;
	
	//return different paramter of nao robot
	//the detail paramter must be modifyed according rsg in server when hetero nao changed
	if( "['ThighRelHip2_Z']" == heteroType) {
	  switch(naoType) {
	    case 0:
	      result = -0.04;
	      break;
	...
	} else if( "['AnkleRelShank_Z']" == heteroType) {
	  switch(naoType) {
	    case 0:
	      result = -0.055;
	      break;
	...
	} else if( "['lj5_max_abs_speed']" == heteroType ) {
	  switch(naoType) {
	    case 0:
	      result = 6.1395;
	      break;
	...
	} else if( "['lj6_max_abs_speed']" == heteroType ) {
	  switch(naoType) {
	    case 0:
	      result = 6.1395;
	      break;
	...
	} else if( "['ElbowRelUpperArm_Y']" == heteroType ) {
	  switch(naoType) {
	    case 0:
	      result = 0.07;
	      break;
...
// new device invoke add by allen 2012-6-27
        IMPORT_DEVICE(AgentCollisionHandler);
        ...
        throw(ClassException<RSGImporter>)
    {
        ...
        while( sexp ){
            ...
            const sexp_t* t = sexp->list;
	    //cout<<"test in import device fullname: "<<t->val<<endl;
            string funName;
    ...
    DEFINE_DEVICE_INVOKE(Device)
    {
        ...
        ///inform
        //add by yao 2013/5/19 because server add hetero nao type
        //the code below can be combined with code upper, 
        //but in order to show how to modify this part to adjust server change
        //thus I write code below
        //if there another attribute changed in agent state, must modify in this function
        if ( "setRobotType" == func) {
	    string val;
            calValue(sexp, defs, val);
            dev->setName(val);
            return true;
...
////////////////////////////////////////////////////////
    ...
    DEFINE_DEVICE_INVOKE(RestrictedVisionSensor)
    {
        ...
        //new device invoke add by yao	2013/06/13
	CALL_BOOL_SET_FUNC(setSenseMyOrien);
    ...
    DEFINE_DEVICE_INVOKE(AgentCollisionHandler)
    {
         CALL_PARENT_METHODS(CollisionHandler);
    }
