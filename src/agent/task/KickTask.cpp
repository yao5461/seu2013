...
KickTask::KickTask( const string& firstTaskName, Task* primary ,int errAng):
	Task(-1, primary )
{
	mIsFirstTime=true;
...
///	mDuration=FAT.calTaskTime(mRaiseFootTask) + 0.6;
	mDuration=FAT.calTaskTime(mRaiseFootTask) +0.2;	
	errAngKickTask=errAng;
	if(isFirstTime)
	{
		isFirstTime=false;
	}
...
shared_ptr<Action> KickTask::perform()
{
	...
	{
		return FAT.controlPreferThan("squat", "*",errAngKickTask);
	}
	else
	{
		return FAT.controlPreferThan(currentTaskName,"*",errAngKickTask);
	}
