...
bool Agent::init()
{
	...
	//modify in 2013/05/23 by yao
// 	if( "rsg/agent/nao/nao.rsg" == FM.getMy().robot ) {
// 	  cout<<"Create Robot: "<<FM.getMy().robot<<"\nHetero type: 0"<<endl;
// 	  COMM<<"(syn)(scene "<<FM.getMy().robot<<")";
// 	} else {
// 	  cout<<"Create Robot: "<<FM.getMy().robot<<"\nHetero type: "<<FM.getMy().heteroType<<endl;
// 	  COMM<<"(syn)(scene "<<FM.getMy().robot<<' '<<FM.getMy().heteroType<<")";
// 	}
	
	cout<<"Create Robot: "<<FM.getMy().robot<<endl;
	//COMM<<"((syn))(scene rsg/agent/nao/nao.rsg)(syn)";
	COMM<<"(syn)(scene rsg/agent/nao/nao.rsg)(syn)";
	COMM.send();
...
void Agent::calculateNextActTime()
{
	...
	changeXTime(mNextActTime, 5000000);
