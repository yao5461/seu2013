...
#include "robot/humanoid/NaoHetero1.h"
#include "robot/humanoid/NaoHetero2.h"
...
namespace configuration{
    ...
    void Configuration::init(int argc, char* argv[])
    {
        ...
        } else if ( "rsg/agent/nao/nao_hetero.rsg" == robotName ) {   //added by yao in 2013/05/23
	    if( 1 == mFormation.getMy().heteroType ) {
	      ROBOT.init<robot::humanoid::NaoHetero1>();
	    } else if( 2 == mFormation.getMy().heteroType ) {
	      ROBOT.init<robot::humanoid::NaoHetero2>();
	    } else {
	      ROBOT.init<robot::humanoid::Nao>();
	    }
	}
